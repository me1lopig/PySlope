...
# ==========================================
def generar_reporte(height, angle, nf, tabla_suelos, fos, fig_plotly, calc_options):
    doc = Document()
    ...
    # 2. Parámetros de Cálculo (NUEVO EN EL REPORTE)
    doc.add_heading('2. Parámetros de Cálculo', level=1)
    p2 = doc.add_paragraph()
    p2.add_run(f"• Dovelas (Slices): {calc_options['slices']}\n")
    p2.add_run(f"• Iteraciones de Búsqueda: {calc_options['iterations']}\n")
    p2.add_run(f"• Tolerancia: {calc_options['tolerance']}")
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        # --- NUEVA SECCIÓN: OPCIONES DE CÁLCULO ---
        with st.expander("4. ⚙️ Opciones Avanzadas de Cálculo"):
            st.caption("Controla la precisión del método de Bishop")
            
            slices = st.number_input(
                "Número de Dovelas", 
                min_value=5, max_value=100, value=25, 
                help="Cantidad de rebanadas verticales. Más dovelas = más precisión pero más lento."
            )
            
            iterations = st.number_input(
                "Iteraciones de Búsqueda", 
                min_value=500, max_value=10000, value=2500, step=500,
                help="Cantidad de círculos de falla aleatorios a probar."
            )
            
            tolerance = st.number_input(
                "Tolerancia Convergencia", 
                min_value=0.001, max_value=0.1, value=0.005, format="%.3f",
                help="Diferencia máxima permitida entre iteraciones del FOS."
            )
            
            max_iter = st.number_input(
                "Máx. Iteraciones (Solver)", 
                min_value=10, max_value=200, value=50,
                help="Límite de intentos para resolver la ecuación de Bishop."
            )
    ...
    if st.button("Calcular Factor de Seguridad", type="primary"):
        ...
        try:
            ...
            # --- 1. APLICAR OPCIONES DE CÁLCULO (TU PETICIÓN) ---
            ...
            # --- 3. EJECUTAR ---
            with st.spinner(f"Analizando {iterations} superficies de falla con {slices} dovelas..."):
                slope.analyse_slope()
            ...
            with col_res1:
                ...
                # Empaquetamos opciones para el reporte
                opciones_reporte = {
                    'slices': slices, 
                    'iterations': iterations, 
                    'tolerance': tolerance
                ...
                with st.spinner("Creando reporte..."):
                    archivo_docx = generar_reporte(height, angle, nf_prof, tabla, fos, fig, opciones_reporte)
