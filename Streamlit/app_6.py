...
def main():
    st.title("⛰️ Análisis de Estabilidad (Entrada por Potencia)")
    ...
    # ==========================================
    # 3. TABLA DE ESTRATOS (MODIFICADA: POTENCIA)
    # ==========================================
    with st.expander("📝 Editar Capas y Colores", expanded=True):
        # CAMBIO CLAVE: Usamos 'Potencia' en lugar de 'Fondo'
        # Ejemplo: Relleno de 5m. Debajo, Arcilla de 7m (llegando a 12m de profundidad total).
        ...
        tabla = st.data_editor(
            ...
            column_config={
                ...
                # Configuramos la columna Potencia
                "Potencia": st.column_config.NumberColumn(
                    "Potencia (m)", 
                    format="%.2f", 
                    min_value=0.1, 
                    help="Espesor vertical del estrato"
                ),
    ...
    if st.button("Calcular Factor de Seguridad", type="primary"):
        ...
        try:
            ...
            # Pyslope necesita la profundidad acumulada (fondo del estrato), 
            # así que la calculamos sumando las potencias.
            profundidad_actual = 0.0
            
            for i, row in tabla.iterrows():
                profundidad_actual += row["Potencia"] # Sumamos el espesor actual
                ...
                mats.append(Material(
                    ...
                    cohesion=row["c"],
                    depth_to_bottom=profundidad_actual, # Usamos el valor acumulado
                    name=row["Material"],
                    color=color_tecnico
                ))
